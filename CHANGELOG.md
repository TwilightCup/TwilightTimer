...
- **Details:**
  - Added an in-game dev console command set (`twi ...`, opened with `~` / `F1`) so every feature — timer, reset/retry, settings, tags, HUD/layout, localization, presets, subsegments, markers, validity flags, and LevelCollections integration — can be inspected and tested from inside the game. See `TESTS.md`.
