...
namespace HSRTimer
{
    ...
    /// through HSRTimerApi (and later the ITimerProvider adapter); the engine
    ...
    public static class RoundTracker
    {
        ...
        public static bool StartRound(string roundId, bool isSingleProject, int retryCount, IEnumerable<string> tags)
        {
            ...
            if (engine == null)
            {
                Plugin.Logger.LogWarning("HSRTimer: StartRound ignored — engine not ready.");
                return false;
            }
            RoundActive = true;
            RoundId = roundId;
            ...
            SetRoundTags(tags);
            Plugin.Logger.LogInfo($"HSRTimer: round '{roundId}' started ({(isSingleProject ? "SINGLE" : "MULTI")}, retry={retryCount}).");
            return true;
        }
        ...
        public static void StopRound()
        {
            ...
            ResolvePendingExit();
            Plugin.Logger.LogInfo("HSRTimer: round stopped.");
        }
        ...
        public static bool SetRoundTags(IEnumerable<string> tagIds)
        {
            ...
            if (!MatchMode.Active)
            {
                Plugin.Logger.LogWarning("HSRTimer: SetRoundTags ignored — not in match mode (T5.1).");
                return false;
            }
            ...
            if (tagIds != null)
            {
                foreach (var id in tagIds)
                {
                    ...
                    {
                        Plugin.Logger.LogWarning($"HSRTimer: ignoring unsupported round tag '{id}' (T5.5).");
                        continue;
                    }
