...
namespace TwilightTimer
{
    ...
    public static class MatchCheckpointPenalty
    {
        ...
        public static void OnSkipViolation(int fromCp)
        {
            ...
            try
            {
                ...
                Plugin.Logger.LogInfo(
                    $"TwilightTimer: match checkpoint-skip penalty — progress rolled back to CP {target}; load the save point from the pause menu to clear the mark.");
            }
            catch (System.Exception ex)
            {
                Plugin.Logger.LogWarning($"TwilightTimer: checkpoint rollback failed: {ex.Message}");
            }
        }
        ...
        public static void OnSaveLoaded()
        {
            if (!Pending) return;
            Pending = false;
            Plugin.Logger.LogInfo("TwilightTimer: match checkpoint-skip penalty cleared (save point loaded).");
        }
